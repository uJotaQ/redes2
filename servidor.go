...
func (f *FSM) Apply(log *raft.Log) interface{} {
	...
	case "CLEANUP_ROOM_RAFT":
        var data struct {
            SalaID  string `json:"salaID"`
            P1Login string `json:"p1Login"`
            P2Login string `json:"p2Login"`
        }
        if err := json.Unmarshal([]byte(cmdPayload), &data); err != nil {
            return fsmApplyResponse{err: fmt.Errorf("FSM: falha ao decodificar cleanup_room")}
        }
        
        mu.Lock()
        defer mu.Unlock()

        // Remove os jogadores do 'playersInRoom' replicado
        delete(playersInRoom, data.P1Login)
        delete(playersInRoom, data.P2Login)
        // Remove a sala do 'salas' replicado
        delete(salas, data.SalaID)

        logger.Printf("[FSM] Sala %s limpa com sucesso (jogadores %s e %s removidos).", data.SalaID, data.P1Login, data.P2Login)
        return fsmApplyResponse{response: "OK"}
        ...
        case map[string]interface{}: // Sucesso (NotePlayed ou GameOver)
             ...
             if gameEnded, _ := resultData["gameEnded"].(bool); gameEnded {
				 ...
				 notifyPlayer(p2Login, protocolo.Message{Type: "GAME_OVER", Data: gameOverMsgP2})
                  ...
                  // Dispara o comando de cleanup para a FSM (fire-and-forget)
                 logger.Printf(ColorGreen+"Disparando cleanup da sala %s para a FSM."+ColorReset, salaID)
                 cleanupPayload, _ := json.Marshal(map[string]string{
                    "salaID":  salaID,
                    "p1Login": p1Login,
                    "p2Login": p2Login,
                 })
                 cmd := []byte("CLEANUP_ROOM_RAFT:" + string(cleanupPayload))
                 _ = raftNode.Apply(cmd, 100*time.Millisecond) // Fire-and-forget
                ...
                if salaID != "" {
                     ...
                     if gameEnded, _ := resultData["gameEnded"].(bool); gameEnded {
                          winner := resultData["winner"].(string)
                           ...
                           // Dispara o comando de cleanup para a FSM (fire-and-forget)
                           logger.Printf(ColorGreen+"[API REST] Disparando cleanup da sala %s para a FSM."+ColorReset, salaID)
                           cleanupPayload, _ := json.Marshal(map[string]string{
                            "salaID":  salaID,
                            "p1Login": p1Login,
                            "p2Login": p2Login,
